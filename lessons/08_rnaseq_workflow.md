...
Originally developed over a decade ago, GMAP and GSNAP exhibit ongoing development to address continuing advancements in technology. Currently, for the straightforward alignments, GSNAP provides efficient mapping utilizing suffix arrays. For more difficult alignments, which the suffix arrays can miss, GSNAP uses a hash table method. Finally, for reads not fully aligning using these methods, GSNAP has integrated the GMAP algorithm to take these partial alignments and use more computationally intensive methods to determine the most accurate alignments. This method was optimized to more accurately identify multiple indels and splice sites within a single read. For this reason, this tool is often recommended for long reads, such as those generated by Pacific Biosciences. For more details regarding the GSNAP method please see the [latest publication](https://link.springer.com/protocol/10.1007%2F978-1-4939-3578-9_15).
...
> - If planning to perform other types of analyses, like SNP analyses, intron retention analyses, or fusion analyses, among others, this versatile tool has specific parameters to change to optimize its use with these types of data.

...
Since we are going to be working with this data on the remote server (the new Genentech cluster): `rosalind.gene.com` , we first need to log onto the server. After we're logged on, we will each make our own copy of the example data folder.
